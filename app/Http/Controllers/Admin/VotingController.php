...
class VotingController extends Controller
{
    ...
    public function votingRecord($election_id)
    {
        ...
        if($election)
        {
            ...
            $voting_records = null;$candidates= null;$vote_count=null;$ques=null;$ans_records=null;$ans_summary=null;
    ...
    public function votingResult($election_id)
    {
        ...
        {
            $elections = $election_modal->electionWithoutCurrent($election_id);
            $voting_count = null; $voting_reject_count = null;$not_voted_count = null; $voting_results = null ; $data = null; $candidates = null; $questions = null; $ques_data = null;
            ...
            if($election->ques_flag == 1)
            {
                $questions = Question::where('election_id', '=', $election_id)->orderby('id', 'asc')->get();

                $ques = new Question();
                $ques_data = $ques->QuesSummary($election_id);
            }
    ...
    public function get_questionList()
    {
        $count = $_GET['count'];
        $election_id = $_GET['election_id'];
        $ques = new Question();
        $result =  $ques->QuesSummary($election_id);
        echo json_encode($result);
    }
    ...
    public function get_ques_result()
    {
        if (request()->ajax()) {
            $election_id = $_GET['election_id'];                                   
            $ques = new Question();
            $DT_data =  $ques->QuesSummary($election_id);            
            return datatables()->of($DT_data)
                ->addIndexColumn()
                ->addColumn('action', function ($DT_data) {
                    $button = '&nbsp;&nbsp;';
                    $button .= '<button type="button" name="detail" id="' . $DT_data->question_id . '" class="detail btn btn-info btn-xs btn-flat"><i class="fas fa-eye"></i> Detail</button>';
                    return $button;
                })
                ->rawColumns(['action'])
                ->make(true);
    ...
    // public function answer($election_id)
    // {
    //     $election_modal = new Election;
    //     $election = $election_modal->electionWithId($election_id);
    //     if($election)
